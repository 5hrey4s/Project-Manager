...
import {  Trash2, Calendar, User, Tag, Paperclip, Calendar as CalendarIcon } from 'lucide-react';

// --- Service Imports ---
import { getTaskDetails, updateTask, deleteTask } from '../services/api';

// --- Type Definitions (Preserving all original types) ---
...
interface TaskDetails {
  ...
  status: string;
  priority: string;
  start_date: string | null; // Dates will be added
...
interface TaskDetailsModalProps {
  taskId: number | null;
  onClose: () => void;
  // No need for other props, it's self-contained now
}
...
export default function TaskDetailsModal({ taskId, onClose }: TaskDetailsModalProps) {
  ...
  // ... (add state for other editable fields like comments if needed)
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        setTask(taskData);
        // Initialize editable state from the fetched data
  ...
  const handleSaveChanges = async () => {
    if (!taskId) return;
    try {
      // --- MODIFIED: Send all editable fields in one update call ---
      await updateTask(taskId, {
        title,
        description,
        start_date: startDate,
        due_date: dueDate,
        // ... (include other fields like priority if they become editable)
      });
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-3 gap-8 h-full">
            ...
            <div className="col-span-2 pr-8 border-r overflow-y-auto">
              ...
              {/* --- Existing Comments Section (Preserved) --- */}
              <div className="mt-8">
                <h4 className="font-semibold mb-4">Comments</h4>
                <div className="space-y-4">
                  {task.comments.map(comment => (
                    <div key={comment.id} className="text-sm">{comment.author_name}: {comment.content}</div>
                  ))}
                  {/* Future: Add a form to post new comments here */}
                </div>
              </div>
            </div>
            ...
            <aside className="col-span-1 space-y-6">
              ...
              <div className="pt-6 border-t">
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This will mark the task as deleted. This action can be undone by an administrator.</AlertDialogDescription></AlertDialogHeader>
